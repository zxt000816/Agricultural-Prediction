...
def params_extractor(_dict):
    ...
    raw = _dict.get('raw')
    return base_dir, product, attribute, raw
...
def support_vector_regression(X, y, search=False, save=False, **params):
    base_dir, product, attribute, raw = params_extractor(params)
    ...
    file_path = file_saver(base_dir, product, attribute, 'SVR', raw)
    file_existed = os.path.exists(file_path)
...
def random_forest(X, y, search=False, save=False, **params):
    ...
    file_path = file_saver(base_dir, product, attribute, 'RF', raw)
    file_existed = os.path.exists(file_path)
...
def gradient_boosting(X, y, search=False, save=False, **params):
    ...
    file_path = file_saver(base_dir, product, attribute, 'GB', raw)
    file_existed = os.path.exists(file_path)
