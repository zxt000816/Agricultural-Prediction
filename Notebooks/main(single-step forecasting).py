...
for i, experiment in enumerate(all_experiments):
    print("*"*50 + f"  {i}/{len(all_experiments)+1}  "  + "*"*50)
    ...
    X_train, X_test, y_train, y_test, input_scaler, output_scaler = data_split(df, input_cols=input_features, output=target, train_size=train_size, scaling=True, **ml_split_params)
